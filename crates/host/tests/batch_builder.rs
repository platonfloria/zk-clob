use zk_clob_core::{
    Account, AssetBalance, BatchMetadata, ExchangeConfig, FeeConfig, MarketConfig, Order, Side,
    settle_batch,
};
...
fn builds_and_applies_a_subset_account_witness() {
    ...
    let mut builder =
        BatchBuilder::new(&state, &config, BatchMetadata::new(1, 31_337, EXCHANGE, 0));
    assert!(matches!(
